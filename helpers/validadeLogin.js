...
function validarFunc(req, res, next) {
	...
	if (!authHeader) {
		return res.status(401).json({ status: false, message: "Não tem header" });
	}

	const token = authHeader.split(" ")[1]; // Extrair o token do cabeçalho "Authorization"

	if (!token) {
		return res.status(401).json({ status: false, message: "Tem coisa errado" });
	}
...
function validarGerente(req, res, next) {
	...
	try {
		...
		if (decoded.cargo != "Gerente") {
			return res.status(403).json({ status: false, message: "Usuário não é gerente" });
		}
		req.id = decoded.id;
		req.cargo = decoded.cargo;
		next();
